...
+(function($) {
    ...
    if (this.options.onChange !== null) {
      if (typeof this.options.onChange !== 'function') {
		...
		this.datetime = this.isTimestamp(
      this.element.val())? this.parseValue(this.element.val()): this.now();
	};
  ...
  DatetimePicker.prototype.get = function() {
    ...
    if (this.options.showAll || this.options.showHours) {
      result["hours"] = this.element
        .find('[data-datetime-part="hours"]').val();
    }

    if (this.options.showAll || this.options.showMinutes) {
      result["minutes"] = this.element
        .find('[data-datetime-part="minutes"]').val();
    }

    if (this.options.showAll || this.options.showSeconds) {
      result["seconds"] = this.element
        .find('[data-datetime-part="seconds"]').val();
    }
	...
	DatetimePicker.prototype.toJSON = function(date) {
		/** Converts to JSON */
		return {
			date: this.pad(date.getFullYear(), 4) + "-" +
    ...
    var checks = this.get();
		...
		for (var check in checks) {
			...
			if (checks[check] > max) {
				$('[data-datetime-part="'+check+'"]').val(max);
			}
			if (checks[check] < min) {
        $('[data-datetime-part="'+check+'"]').val(min);
			}
		}
	};
    ...
    var value = this.get();
		...
		var hours = (value.hours !== null)? this.pad(value.hours, 2): '00';
    var minutes = (value.minutes !== null)? this.pad(value.minutes, 2): '00';
    var seconds = (value.seconds !== null)? this.pad(value.seconds, 2): '00';
    ...
    var hidden = $('[name="' + this.input_name + '"]')
      .val(result_string);
	...
	function Plugin(options) {
		return this.each(function() {
			...
			if (typeof options !== 'undefined') {
				if (!data) $(this).data('DatetimePicker', (data = new DatetimePicker(this, options)));
			} else {
				if (!data) $(this).data('DatetimePicker', (data = new DatetimePicker(this)));
			}
